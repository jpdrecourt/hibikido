...
class TestHibikidoDatabase:
    ...
    def test_add_recording(self, db, sample_recording_path):
        ...
        result = db.add_recording(
            ...
            metadata={"description": "Birds singing in the morning forest"}
)
    ...
    def test_add_duplicate_recording(self, db, sample_recording_path):
        ...
        # Add first recording
        db.add_recording(sample_recording_path, metadata={"description": "Description 1"})
        
        # Try to add duplicate
        result = db.add_recording(sample_recording_path, metadata={"description": "Description 2"})
    ...
    def test_get_all_recordings(self, db):
        ...
        for i, path in enumerate(paths):
            db.add_recording(path, metadata={"description": f"Description {i}"})
    ...
    def test_add_segment(self, db, sample_recording_path):
        ...
        db.add_recording(sample_recording_path, metadata={"description": "Test recording"})
    ...
    def test_full_path_based_workflow(self, db):
        ...
        for path in recording_paths:
            success = db.add_recording(
                path=path,
                metadata={"description": f"Recording: {path.split('/')[-1]}"}
            )
...
class TestDataFixtures:
    ...
    def create_bird_recording_set(db: HibikidoDatabase):
        ...
        db.add_recording(
            path=recording_path,
            metadata={"description": "Dawn chorus recorded in oak woodland, spring morning"}
        )
